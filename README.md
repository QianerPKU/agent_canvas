...
`workspace.json` 使用带版本的 `agent-canvas/workspace` schema，同时保存项目名称和 id，因此项目目录本身可以独立识别。打开页支持直接选择任意已有项目文件夹；加载时会校验并重定位项目内部路径，项目目录外的共享资源需要再次授权。加载成功后会把项目重新登记到列表，默认项目列表根目录下的项目即使索引丢失也会自动被发现。项目列表中的删除按钮会在确认后永久删除整个项目文件夹及其中的 repo/worktree、画布状态和项目级资源；后端会拒绝跨站请求、缺少删除意图头或仍有活动 agent 的删除操作。
