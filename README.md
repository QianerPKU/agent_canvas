...
`workspace.json` 同时保存项目名称和 id，因此项目目录本身可以独立识别。打开页支持直接选择任意已有项目文件夹；加载成功后会把它重新登记到项目列表。默认项目列表根目录下的项目即使索引丢失也会自动被发现。项目列表中的删除按钮会在确认后永久删除整个项目文件夹及其中的 repo/worktree、画布状态和项目级资源。
