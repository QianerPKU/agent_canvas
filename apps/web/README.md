...
| `src/nodes/TurnNode.tsx` | 自定义节点（一轮）：状态徽标 + provider/model 选择 + 输出滚动区；最新节点提供 Compact/Terminate，普通状态提供输入、停止或 fork 控件 |
...
- `nodes/TurnNode.test.tsx`：各状态徽标、模型选择、Compact/Terminate 与 fork 控件交互（jsdom + Testing Library）
