...
- 顶栏“设置”打开当前 Canvas 项目的设置；“完全权限模式”开启后，后端直接允许所有授权请求，并放行已挂起授权，防止对话卡在审批上。它不会自动回答普通问题。
- “工作文档维护”开启后，Agent 会持续维护不进入 Git 的 branch 隔离详细文档和共享概要文档，两份固定索引随每轮作为参考文件提供。设置按 Canvas 项目持久化，打开或新建项目后前端会重新读取，避免沿用上一个项目的值。
