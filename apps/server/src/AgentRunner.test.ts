...
describe("AgentRunner 生命周期", () => {
  ...
  it(
    "does not let a hanging explicit-steer fallback interrupt block dispatch or exact termination",
    async () => {
      const output = new AsyncMessageQueue<SdkMessage>();
      const interruptStarted = deferred();
      const neverInterrupts = deferred();
      const runner = new AgentRunner("hanging-steer-interrupt-agent", {
        query: () => ({
          [Symbol.asyncIterator]: () => output[Symbol.asyncIterator](),
          interrupt: async () => {
            interruptStarted.resolve();
      ...
      output.push(SYSTEM_INIT);
      await flush();
      const dispatch = runner.steer("queued steer");
      await expect(
        Promise.race([
