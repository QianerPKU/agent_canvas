...
3. Active agents on the source branch receive the review request. If there are none, the oldest same-branch idle agent is started with the request. Review agents must return the fixed JSON schema; invalid JSON is retried once, then recorded as `blocked`.
...
`PullRequestFlowManager.test.ts` covers source review approval, PR creation, target review approval, merge authorization, invalid JSON retry failure, timeout behavior, per-stage branch queueing, blocked-delivery cancellation and timeout, zero-reviewer deferral, idle-reviewer auto-start with active-reviewer preference, natural-id tie-breaking for equal creation times, FIFO release, persistence recovery, and changed-file resolution.
