...
When persisted canvas state is restored, an in-progress review is converted back to `queued`, its old deadline is discarded, and its previous request remains as audit history. Each queued stage persists a sequence allocated by the shared queue, so PR and sync jobs with identical timestamps retain their original total order regardless of manager import order. Older snapshots without a sequence remain loadable and receive one during restoration. The server first restores agents, prompts, and layout, then atomically rebuilds the PR and sync portions of the shared queue. An eligible head starts with a fresh request and deadline; a head without an active reviewer stays queued instead of being vacuously approved. This also ensures no review prompt or partial state save occurs while project restoration is incomplete.
