...
3. Active agents on the source branch receive the review request. If there are none, the oldest same-branch idle agent is started with the request. Each agent submits its decision through `POST /api/pr-flows/:id/reviews` as an intermediate tool call, then continues its existing reply. A legacy final-result JSON response is still accepted; invalid legacy JSON is retried once, then recorded as `blocked`.
...
6. The target merge stage enters the shared queue for `targetBranch`; active target-branch agents, or the oldest idle fallback when no active agents exist, receive the request only after that stage acquires the branch.
...
The reserved prefix is also redacted from every non-capability PR-flow string before it can reach branch/file resolution, canonical flow state, another agent's prompt, persistence, or export. Direct review callbacks preserve their dedicated `reviewToken` only for validation while redacting the agent id, summary, and review arrays. A `pr-created` callback authenticates its dedicated `completionToken` before its PR metadata and idempotency fingerprint are canonicalized. Legacy result JSON and imported snapshots pass through the same state boundary. Every review, retry, authorization, failure, and closure prompt builder also creates a fresh redacted copy immediately before interpolation, protecting cross-agent delivery even if a retained `get()`/`list()` snapshot reference was mutated after canonical storage. Newly issued review and completion tokens are passed separately and injected only after the rest of the prompt data is canonical; the finished prompt is never run through the redactor, so the intended recipient still receives a usable callback token without an echoed token crossing flow boundaries.
...
`PullRequestFlowManager.test.ts` covers private callback capabilities, impersonation rejection, validation and idempotency, restored authorization token rotation, queued-input result isolation, all-reviewer freezes and close releases, legacy result JSON compatibility, source review approval, PR creation, target review approval, merge authorization, invalid JSON retry failure, timeout behavior, per-stage branch queueing, blocked-delivery cancellation and timeout, zero-reviewer deferral, idle-reviewer auto-start with active-reviewer preference, natural-id tie-breaking for equal creation times, FIFO release, persistence recovery, and changed-file resolution.

`PullRequestFlowManager.integration.test.ts` creates a real temporary git repository and bare remote, uses `WorkspaceManager` to clone `main`, `feature/pr-flow`, and `feature/other`, starts multiple fake agents, and verifies review delivery across source and target branches. It also exercises the real `AgentManager` idle auto-start path, deterministic oldest-idle selection, and duplicate-delivery protection when the reviewer reaches `running`.

`server.test.ts` covers the HTTP fork wake-up path end to end: an idle fork on the target branch retries a deferred PR review, while the fork's later `running` event cannot create a second request or deliver a duplicate prompt.
